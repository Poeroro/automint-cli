...
def show_detect_result(data: dict):
    ...
    for i, t in enumerate(tiers, 1):
        price = f'{t["price"]} ETH' if t['price'] > 0 else '[green]FREE[/green]'
        ...
        table.add_row(
            str(i), t.get('name', '?'), price, status,
            starts_in if status_raw and status_raw.startswith('scheduled:') else '[dim]—[/dim]'
        )
...
def show_cost_estimate(est: dict):
    ...
    console.print(f'  Mint Price:  [cyan]{est["price_eth"]:.6f} ETH[/cyan]')
    console.print(f'  Gas Units:   {est["gas_units"]:,}')
    console.print(f'  Gas Price:   {est["gas_price_gwei"]:.2f} Gwei')
    console.print(f'  Gas Cost:    [yellow]{est["gas_cost_eth"]:.6f} ETH[/yellow]')
    console.print(f'  [bold]Total:      [cyan]{est["total_eth"]:.6f} ETH[/cyan][/bold]')
...
def show_report(report: dict, chain: str = ''):
    ...
    if status == 'success':
        ...
        console.print(Panel.fit(
            ...
            f'[bold]Gas Fee:[/bold] {report["gas_cost_eth"]:.6f} ETH\n'
            f'[bold]Total:[/bold]  [cyan]{report["total_cost_eth"]:.6f} ETH[/cyan]',
            border_style='green', padding=(1, 2)
        ))
